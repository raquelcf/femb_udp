...
for i in {0..7}; do
  ...
  for j in {0..15}; do
    ...
    list=""
    for k in {1..100}; do
	rm temp.dat; thisfile="asic"${i}"chan"${j}".txt"
	python get_data.py | gawk '{if (NR>1} {print $2}}' >> temp.dat
	cp temp.dat scandata/${thisfile}
        chanstats=`tail -500 temp.dat| perl -e 'use List::Util qw(max min sum); @a=();while(<>){$sqsum+=$_*$_; push(@a,$_)}; $n=@a;$s=sum(@a);$a=$s/@a;$m=max(@a);$mm=min(@a);$std=sqrt($sqsum/$n-($s/$n)*($s/$n));$mid=int @a/2;@srtd=sort @a;if(@a%2){$med=$srtd[$mid];}else{$med=($srtd[$mid-1]+$srtd[$mid])/2;};print "sum:\t$s\navg:\t$a\nstd:\t$std\n\med:\t$med\n\max:\t$m\min:\t$mm";'`
        list="${list}
